...
from paperfarm.event_journal import now_iso
    ...
    *,
    ...
    if not cfg.bootstrap_auto_prepare:
        ...
        if on_prepare_event is not None:
            from paperfarm.research_events import PrepareCompleted
    ...
    if on_prepare_event is not None:
        from paperfarm.research_events import PrepareStarted
    ...
    if state.get("errors"):
        ...
        write_bootstrap_state(state_path, state)
        if on_prepare_event is not None:
            from paperfarm.research_events import PrepareFailed
    ...
    for step_name in _step_names():
        ...
        if step_name == "data":
            ...
            if expected_paths and all(item.get("exists") for item in expected_paths):
                ...
                continue
        if on_prepare_event is not None:
            from paperfarm.research_events import PrepareStepStarted
        ...
        write_bootstrap_state(state_path, state)
        if on_prepare_event is not None:
            from paperfarm.research_events import PrepareStepCompleted
