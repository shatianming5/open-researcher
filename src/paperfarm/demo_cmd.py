...
def _setup_demo_repo(repo: Path) -> None:
    ...
    subprocess.run(
        ["git", "config", "user.email", "demo@paperfarm.dev"],
        cwd=str(repo),
        capture_output=True,
    )
    subprocess.run(
...
def do_demo(serve: bool = False, port: int = 8000) -> None:
    ...
    console.print("[bold green]PaperFarm Demo[/bold green]")
    ...
    if serve:
        try:
            ...
            console.print(
                "[red]textual-serve is not installed.[/red]\nInstall it with: [bold]pip install PaperFarm[serve][/bold]"
            )
            return
        ...
        launcher.write_text(
            ...
            "from paperfarm.demo_cmd import _inject_logs\n"
    ...
    console.print(
        ...
        "  [bold]PaperFarm init[/bold]\n"
        "  [bold]PaperFarm run --agent claude-code[/bold]"
    )
