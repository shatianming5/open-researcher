...
| `.research/evaluation.md` | Read/Write | Keep evaluation contract executable |
| `.research/config.yaml` | Read/Write | Backfill `metrics.primary.*` if empty |
| `.research/results.tsv` | Read | Experiment results ledger |
| `.research/research-strategy.md` | Read | Scout strategy output |
| `.research/project-understanding.md` | Read | Scout analysis output |
| `.research/literature.md` | Read | Scout literature review |
...
- For `.research/log.jsonl`, read only the latest lines you need. Do not ingest the whole event log as research context.
- Prefer repo source files, `project-understanding.md`, `research-strategy.md`, `evaluation.md`, `graph.json`, and `results.tsv` over generated runtime chatter.
...
- If `activity.json -> control.paused` is true, wait until unpaused
